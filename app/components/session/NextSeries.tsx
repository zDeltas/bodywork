...
export type NextSeriesProps = {
  exercise: Exercise;
  currentSeriesIndex: number;
};

const NextSeries: React.FC<NextSeriesProps> = ({ exercise, currentSeriesIndex }) => {
  const { theme } = useTheme();
  const { t } = useTranslation();
  const styles = useStyles(theme);

  const upcoming = exercise.series.slice(currentSeriesIndex + 1, currentSeriesIndex + 4);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{t('workout.nextSeries')}</Text>
